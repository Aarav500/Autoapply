...
import { ConfigureSMSSchema, type UserSettings } from '@/types/notifications';
import { createPhoneMapping } from '@/app/api/comms/webhook/twilio/route';

/**
 * PUT /api/comms/sms/configure
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const parsed = ConfigureSMSSchema.safeParse(body);
    if (!parsed.success) {
    ...
    settings.twilioConfigured = enabled;
    settings.notificationPreferences.smsEnabled = enabled;

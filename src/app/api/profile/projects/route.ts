...
import { addProject } from '@/services/profile/profile-service';
import { ProjectCreateSchema } from '@/types/profile';
...
export async function POST(request: NextRequest) {
    try {
        ...
        const profile = await addProject(userId, validatedData);

        logger.info({ userId }, 'Project added via API');
        ...
        logger.error({ error }, 'Add project endpoint error');

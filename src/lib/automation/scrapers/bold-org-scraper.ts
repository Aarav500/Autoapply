...
// Scrape scholarships from Bold.org with profile-specific searches
export async function scrapeBoldOrg(maxResults = 20): Promise<ScrapedScholarship[]> {
    ...
    // Profile-specific search URLs for better matching
    const searchUrls = [
        'https://bold.org/scholarships/?q=computer+science',
        'https://bold.org/scholarships/?q=stem',
        'https://bold.org/scholarships/?q=technology',
        'https://bold.org/scholarships/?q=international+students',
        'https://bold.org/scholarships/?q=asian',
        'https://bold.org/scholarships/?q=indian',
        'https://bold.org/scholarships/', // General fallback
    ];
    ...
    try {
        ...
        bm.log('📚 Scraping Bold.org with profile-specific searches...');

        for (const searchUrl of searchUrls) {
            if (scholarships.length >= maxResults) break;

            bm.setStep(`Searching: ${searchUrl.split('?q=')[1] || 'all'}`, searchUrls.indexOf(searchUrl) + 1, searchUrls.length);
            ...
            try {
                ...
                bm.log(`⚠ Failed to search ${searchUrl}: ${err}`);
            }
        } // End of for loop over searchUrls

        bm.setStep('Complete', searchUrls.length, searchUrls.length);
...
export async function discoverScholarships(): Promise<number> {
    ...
    const scholarships = await scrapeBoldOrg(30); // Increased to 30 for better coverage
    let addedCount = 0;
